...
export function ProjectPage() {
  ...
  return <article className="container-page pt-10 sm:pt-16">
    ...
    <section className="mt-16 sm:mt-24"><p className="eyebrow">Screenshots</p><h2 className="mt-2 text-2xl font-semibold">Inside the product</h2><div className="mt-7 grid gap-5 md:grid-cols-2">{project.screenshots.map((shot,i)=><figure key={`${shot.src}-${i}`} className="overflow-hidden rounded-xl border border-white/[0.08] bg-panel"><img src={shot.src} alt={shot.alt} loading="lazy" className="aspect-[16/10] w-full object-cover"/><figcaption className="border-t border-white/[0.07] px-4 py-3 text-xs text-zinc-500">{shot.alt}</figcaption></figure>)}</div></section>
